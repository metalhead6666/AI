...
public class HoldOut extends AbstractEvaluationMethod {
     ...
     * @param percentageToTrain The percentage of instances to train.
     */
    public HoldOut(AbstractALAlgorithm algorithm, AbstractDataset dataset,
            double percentageToTrain) {
    ...
    /**
     *
     * @param percent The percentage selected to train.
     */
    ...
    public void evaluate() {
        ...
        try {
            ...
            //One general dataset was specified, then it must be divided into train and test taking 
            //into account the percentage defined
            if (getDataset() != null) {

                DatasetUtils.randomize(((AbstractSampling) getSamplingStrategy()).getRandgen(), getDataset());

                int numInstances = (int) (getDataset().getNumInstances()
                        * percentageToSplit / 100);

                if (!isMultiLabel()) {

                    setTrainDataset(new WekaDataset(getDataset(), 0, numInstances));
                    setTestDataset(new WekaDataset(getDataset(), numInstances,
                ...
                //Resample the instances to construct the labeled and unlabeled set on the training set
                ...
                //Else, An unlabeled and labeled file were specified, then a resample method is not necessary
            }
            ...
            Logger.getLogger(HoldOut.class.getName()).log(
                    Level.SEVERE, null, e);
        }
