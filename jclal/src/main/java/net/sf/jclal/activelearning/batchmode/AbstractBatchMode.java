...
/**
 * Abstract class for batchSize mode strategy
 *
...
public abstract class AbstractBatchMode implements IBatchMode, IConfigure {
    ...
    /**
     * 
     * @param batchSize The number of instances to select in each iteration
     */
    ...
    public void configure(Configuration configuration) {
        ...
        if (batchT <= 0) {
            throw new ConfigurationRuntimeException(
                    "\nIllegal batch size: <batch-size>" + batchT + "</batch-size>"
                    + ". Batch size > 0");
        }
        setBatchSize(batchT);
    }
